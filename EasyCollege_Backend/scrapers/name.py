# EasyCollege_Backend/scrapers/name.py

import os
from bs4 import BeautifulSoup
import requests

def return_data(name=None, password=None):
    loginurl = os.getenv('LOGIN_URL')
    secure_url = os.getenv('MENU_URL')

    # Default payload for regular users
    payload = {
        '__VIEWSTATE':'wfjaEYy0FI1sC/VPcVi24QcJ31uKgMtfa3AcGJvkFG5eYcjfXGwp6jMbiyeOYHyuetWIJaXF3LgTGOWxWGk7HHwSXcbCWViqfnYAteJFkelInRUDFPrCs7G7yLAQ1GpX2js6Ki7c6HXZbkQTvYkIZkxxNG//Sxq0BFgFlqs0PyvLnVXQoBvwSFgztVKjrBhS7P6PyBMb9dQj88s3wTN9TvwlH1v5wm9euE12PWvSN4A=',
        '__VIEWSTATEGENERATOR':'E64D2FFE',
        '__EVENTVALIDATION':'',
        'rdolst':'S',
        'txtusercheck': name if name else "",
        'txtpwdcheck': password if password else "",
        'abcd3':'Login'
    }

    # Special payload for the user '23z310'
    if name == '23z310':
        payload={
            '__VIEWSTATE':'',
            '__VIEWSTATEGENERATOR':'E64D2FFE',
            '__EVENTVALIDATION':'',
            'rdolst':'P',
            'txtusercheck': name if name else "",
            'txtpwdcheck': password if password else "",
            'abcd3':'Login'
        }

    try:
        with requests.session() as s:
            s.post(loginurl, data=payload)
            r = s.get(secure_url)
            soup = BeautifulSoup(r.content, 'html.parser')
            result_table = soup.find('span', {'id': 'Title1_LblStaffName'})
            
            if result_table and result_table.text.strip():
                return result_table.text.strip()
            else:
                return None
    except requests.exceptions.RequestException as e:
        print(f"Error in name scraper: {e}")
        return None